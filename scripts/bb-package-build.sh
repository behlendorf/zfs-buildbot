...
#
# Due to a missing dependency in the spec file for the moment python2
# needs to be manually installed for CentOS 7 mods.
case "$BB_NAME" in
CentOS*)
	if cat /etc/centos-release | grep -Eq "7."; then
		mock --init
		mock --install python2
		mock --no-clean $MOCK_OPTIONS zfs-*.src.rpm || exit 1
	else
